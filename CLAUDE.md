...
them here). The reused pins are frozen at `com.IvanMurzak.ReflectorNet` **5.3.3** and
`com.IvanMurzak.McpPlugin` **7.5.0** (in `Godot-MCP.csproj`, mirrored by `Godot-MCP.Tests/` and the infra
