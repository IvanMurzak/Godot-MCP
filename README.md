...
| **script** | `script-read`, `script-update-or-create`, `script-delete`, `script-attach-to-node` | CRUD on C# (`.cs`) and GDScript (`.gd`) files, plus attaching a script to a node. (No dynamic code execution — that is intentionally out of scope.) |
