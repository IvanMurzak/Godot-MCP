...
| **Version** | `0.23.0` (must match `addons/godot_mcp/plugin.cfg` `version=` and the `v0.23.0` tag) |
| **Repository host** | `GitHub` |
| **Repository URL** | `https://github.com/IvanMurzak/Godot-MCP` |
| **Issues URL** | `https://github.com/IvanMurzak/Godot-MCP/issues` |
| **Download Commit** | the commit hash the `v0.23.0` tag points at — get it with `git rev-list -n1 v0.23.0` (a hash, not the tag name) |
| **License** | `Apache-2.0` |
...
project's .csproj must declare the two NuGet package references the addon depends on:
  com.IvanMurzak.ReflectorNet  version 5.4.1
  com.IvanMurzak.McpPlugin     version 8.4.0
